...
class RenRenTask:
    ...
    def runningTask(self):
        ...
        try:
            ...
            #sort the jobs
            jobslist=[]
            jobslist=sorted(self.jobs.items(),key=lambda x:x[1],reverse=True)
                ...
                while value<=99999 and not renren.endProgram:
                    ...
                    if not renren.endProgram:
                        try:
                        	...
                        	self.Count_successiveFail+=1
                        	print("第%d次timeout"%self.Count_successiveFail)
                        	if self.Count_successiveFail<=4:
                        		value+=1
                        		continue
                        	else:
                        		raise e
                            ...
                            if value>=99999:
                                finishedidJob=util.loadFormJson(self.oldjob)
                                finishedidJob[currentkey]=value
                                util.dumpToJson(finishedidJob,self.oldjob)
                else:
    ...
    while True:
        if os.path.exists(config.taskfailFile):
            try:
                ...
                print("任务重启.........")
                if os.path.exists(config.taskfailFile):
                    os.remove(config.taskfailFile)
                    sleepTime*=2
                    if sleepTime>100:
                        sleepTime=10
                    monitorTask(sleepTime)                       
            except Exception as e:
            ...
            if os.path.exists(config.tasksuccessFile):
                ...
                print("检测程序退出...")
                exit()                    
            # if not os.path.exists(config.taskfailFile) and not os.path.exists(config.tasksuccessFile):
            #     prompt_relax="%s:休息%d分钟再来检测.......\n"%((time.strftime("%Y-%m-%d %H:%M:%S"),sleepTime))
            #     print(prompt_relax)
            #     time.sleep(60*sleepTime)
            #     monitorTask()
